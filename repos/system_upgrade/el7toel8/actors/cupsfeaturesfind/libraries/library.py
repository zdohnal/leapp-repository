...
from leapp.models import InstalledRedHatSignedRPM, CupsChangedFeatures
...
def initialize(N):
    """
    Initialize N variables to False
...
def find_features(debug_log=api.current_logger().debug,
    ...
    :param func check_path_func: checks if the directory exists
    ...
    if (
        ...
        error_log('Core CUPS configuration files are missing. CUPS'
                  ' installation is corrupted, terminating the actor.')
        return

    debug_log('Checking if CUPS configuration contains removed features.')

    digest, env, printcap, interface, certkey, include = initialize(6)
    ...
    if sum(map(bool, [interface, digest, include, certkey,
               env, printcap])) > 0:
