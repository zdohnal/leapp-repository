...
def migrate_certkey(op):
    ...
    for line in lines:
        for name in ['ServerKey', 'ServerCertificate']:
            if line.lstrip().startswith(name):
                certkey_values.append(line.split()[1])
                lines[lines.index(line)] = ''
    ...
    if all(value.startswith('ssl') or value.startswith('/etc/cups/ssl')
           for value in certkey_values):
        return

    if all(v.startswith(certkey_values[0].rsplit('/', 1)[0])
           for v in certkey_values):
        path = certkey_values[0].rsplit('/', 1)[0]
        op.write(CUPSFILES_CONF, 'a',
                 '\n# added by Leapp\nServerKeychain {}\n'.format(path))
    else:
        for value in certkey_values:
            if (not value.startswith('ssl') and
                    not value.startswith('/etc/cups/ssl')):
